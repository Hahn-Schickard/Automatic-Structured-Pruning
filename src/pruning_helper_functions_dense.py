...
def delete_dense_neuron(
    new_model_param, layer_types, layer_output_shape, layer_bias, layer, netstr, neuron
):
    """
    ...
    if layer_types[layer] == "Dense":
        new_model_param[layer][0] = np.delete(
            new_model_param[layer][0], neuron, axis=1
        )  # Weight
        if layer_bias[layer]:
            new_model_param[layer][1] = np.delete(
                new_model_param[layer][1], neuron, axis=0
            )  # Bias
        ...
        for i in range(layer + 1, len(new_model_param)):
            ...
            if layer_types[i] == "Dense":
                ...
                new_model_param[i][0] = np.delete(new_model_param[i][0], neuron, axis=0)
...
def get_neurons_to_prune_l1(layer_params, prun_layer, prun_factor):
    ...
    prun_neurons = sorted(
        range(new_layer_param[0].shape[-1]), key=lambda k: avg_neuron_w[k]
    )[: int((prun_factor * new_layer_param[0].shape[-1]) / 100)]
